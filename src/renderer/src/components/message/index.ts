 ...
 * @param type
 */
const PositionTypes = ['right_bottom', 'top'] as const
export type PositionType = (typeof PositionTypes)[number]

const init = (position: PositionType) => {
...
const destory = () => {
  ...
  const bodyEl = document.querySelector('body')
  if (msgEl) bodyEl?.removeChild(msgEl)
}
